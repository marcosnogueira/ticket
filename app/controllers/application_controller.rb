...
class ApplicationController < ActionController::Base
  ...
  def track hit_data_name, ammount = 0
    session[:tracked_hit_data] ||= []
    unless session[:tracked_hit_data].include? hit_data_name
      ...
      @source = Source.find_or_create_by_domain :domain => (session[:referer] == "DIRECT" ? session[:referer] : URI.parse(session[:referer]).host) if session[:referer]
      ...
      unless current_user && @share_url && @share_url.user_id == current_user.id
        if @share_url
          @hit = Hit.create :hit_data_id => @hit_data.id, :user_id => @share_url.user_id, :resource_id => @share_url.resource_id, :resource_class_name => @share_url.resource_class_name, :source_url_id => @source_url.id, :source_id => @source.id, :ammount => ammount
          session[:tracked_hit_data].push hit_data_name
        end
      end
    end
